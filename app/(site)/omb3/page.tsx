...
export default async function OmbStepThreePage({ searchParams }: OmbStepThreePageProps) {
  ...
  if (!claim.purchasedProduct || !claim.reviewRating || !claim.commentText) {
    redirect(`/omb2?claim=${claim.id}`);
  }

  if (!isHighRating(claim.reviewRating)) {
    redirect(`/omb2/thank-you?claim=${claim.id}`);
  }
