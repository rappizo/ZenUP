...
const DEFAULT_OPENAI_POST_MODEL =
...
function getAllowedExternalReferences(): AllowedExternalReference[] {
  return [
    {
      ...
      reason: "Useful high-authority background reference for healthy-aging context."
    },
    {
      ...
      reason: "Helpful general supplement guidance from an authoritative public-health source."
    }
  ];
}
...
export async function generateSeoPostDraftWithAi(
  ...
  const allowedExternalReferences = getAllowedExternalReferences();
  const schema = {
    type: "object",
...
export async function generateSeoPostImageFromProductReferenceWithAi(
  ...
  formData.append(
    ...
    [
      ...
      "Turn that core product into a premium editorial scene rather than returning a plain cutout or a simple packshot."
    ].join(" ")
  );
