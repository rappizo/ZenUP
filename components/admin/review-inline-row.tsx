...
export function ReviewInlineRow({
  review,
  productSlug,
  bulkFormId
}: ReviewInlineRowProps) {
    ...
    <tr>
      ...
      <td>
        <div className="admin-table__rating-cell">
          ...
          />
          <RatingStars
            rating={Math.max(1, Math.min(5, Number.parseInt(rating, 10) || review.rating))}
            size="sm"
          />
        </div>
      </td>
      ...
      <td className="admin-table__actions">
        <button
          ...
          onClick={() => runAction("update", "Review update failed.")}
          disabled={isPending}
          aria-busy={isPending}
        >
        ...
        {status !== "PUBLISHED" ? (
          <button
            ...
            onClick={() => runAction("approve", "Review approval failed.")}
            disabled={isPending}
          >
            Approve
          </button>
        ...
        <button
          ...
          onClick={() => runAction("delete", "Review delete failed.")}
          disabled={isPending}
        >
          Delete
        </button>
        {message ? (
          <span
            className={
              message === "Saved" || message === "Approved" || message === "Deleted"
                ? "form-note"
